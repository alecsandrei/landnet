...
class RasterTiles:
    ...
    def resample(
        ...
        dest_paths = [
            out_dir / Path(tile_path).name for tile_path in self.tiles['path']
        ]
        ...
        func = partial(
            resample, crs=crs, tile_size=tile_size, resampling=resampling
        )
        with concurrent.futures.ThreadPoolExecutor() as executor:
            results = executor.map(func, self.tiles['path'], dest_paths)
