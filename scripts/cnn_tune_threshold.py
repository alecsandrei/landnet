...
def train_models():
    ...
    hillshade_train_folder = (
        TRAIN_FOLDER / GeomorphometricalVariable.HILLSHADE.value
    )
    hillshade_test_folder = (
        TEST_FOLDER / GeomorphometricalVariable.HILLSHADE.value
    )
    ...
    for threshold in range(0, 100, 5):
        ...
        out_name = (
            MODELS_FOLDER
            / f'{hillshade_train_folder.stem}_{ARCHITECTURE}_threshold{threshold}'.replace(
                '.', '_'
            )
        )
        print(out_name)
        ...
        reclassify_rasters(
            hillshade_test_folder,
            threshold,
            TEST_LANDSLIDE_PERCENTAGE,
        )
        assert hillshade_train_folder.name == hillshade_test_folder.name, (
            hillshade_train_folder.name,
            hillshade_test_folder.name,
        )
        train_func = partial(
            ray_train_wrapper,
            train_folder=hillshade_train_folder,
        )
        tuner = get_tuner(
            train_func,
            name=hillshade_train_folder.name,
            storage_path=MODELS_FOLDER,
        )
        results = tuner.fit()
...
def get_confusion_labels(
    y_true: c.Sequence[int], y_pred: c.Sequence[int]
...
def save_confusion_matrix(
    ...
    # Set axis labels
    plt.xlabel('Prediction label', fontsize=22)
...
def evaluate_models():
    ...
    for threshold in range(0, 100, 5):
        ...
        model_path = (
            MODELS_FOLDER
            / f'{GeomorphometricalVariable.HILLSHADE.value}_{ARCHITECTURE}_threshold{str(threshold).replace('.', '_')}.pt'
        )
        test_folder = TEST_FOLDER / model_path.stem.split('_')[0]
        train_folder = TRAIN_FOLDER / model_path.stem.split('_')[0]
        reclassify_rasters(
            train_folder,
            threshold,
            TRAIN_LANDSLIDE_PERCENTAGE,
        ...
        print((model_path.parent / model_path.stem).with_suffix('.csv'))
        df = pd.read_csv(
            (model_path.parent / model_path.stem).with_suffix('.csv')
        )
        # df = pd.read_csv(
        #     str(model_path.parent / model_path.stem).replace('.pt', '.csv')
        # )
        ...
        df_test_results_labels[f'pred_{model_path.stem}_confusion'] = (
            get_confusion_labels(test_results.true, test_results.pred)
        )
